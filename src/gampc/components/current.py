...
class Person(Gtk.Box):
    ...
    def __init__(self, image_cache, condition=None):
        ...
        self.connect('notify::name', self.notify_name_cb)
    ...
    def notify_name_cb(self, label, param):
        self.clear()

        if not self.name or (self.condition and not self.condition(self.name)):
            self.set_visible(False)
            return

        self.set_visible(True)

        self.pixbuf = self.image_cache[self.name]
        if self.pixbuf:
            self.label.set_visible(False)
            self.image.set_visible(True)
            self.image_label.set_visible(True)
            self.image_label.set_label(self.name)
        else:
...
class Current(component.Component):
    ...
    def __init__(self, *args):
        ...
        self.unit.unit_server.bind_property('current-song', self.artist, 'name',
                                            GObject.BindingFlags.SYNC_CREATE,
                                            lambda x, y: y.get('Artist', ''))
        ...
        self.unit.unit_server.bind_property('current-song', self.performer, 'name',
                                            GObject.BindingFlags.SYNC_CREATE,
                                            lambda x, y: y.get('Performer', ''))
