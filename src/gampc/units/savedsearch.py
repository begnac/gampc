...
class SavedSearch(component.ComponentPanedMixin, songlist.SongList):
    ...
    def action_search_new_cb(self, *args):
        value = self.unit.ssde_struct.edit(self.widget.get_root(), size=self.config.edit_dialog_size._get(), scrolled=True)
        if value:
    ...
    def edit_search(self, p):
        ...
        new_value = self.unit.ssde_struct.edit(self.widget.get_root(), value, size=self.configself.config.edit_dialog_size._get(), scrolled=True)
        if new_value:
    ...
    def action_search_delete_cb(self, *args):
        ...
        dialog = Gtk.Dialog(parent=self.widget.get_root(), title=_("Delete search"))
