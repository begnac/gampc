...
class FieldColumnFactory(Gtk.SignalListItemFactory):
    def __init__(self, name):
        ...
        self.connect('bind', self.bind_cb, name)
    ...
    @staticmethod
    def bind_cb(self, listitem, name):
...
class FieldColumn(Gtk.ColumnViewColumn):
    ...
    def __init__(self, field):
        super().__init__()
        self.field = field
        self.factory = FieldColumnFactory(field.name)
...
class ColumnView(Gtk.ColumnView):
    def __init__(self, model_class, fields, **kwargs):
        ...
        for name in self.fields.order:
            self.cols[name] = FieldColumn(self.fields.fields[name])
...
# class ColumnView(ColumnViewBase):
#     def __init__(self, fields, data_func, sortable):
#         super().__init__(StoreSort if sortable else StoreFilter, fields, data_func, vexpand=True, rubber_banding=True)
